...
//			prop.load(new FileReader("resources/query.properties"));
			String fileName = ProductDAO.class.getResource("/query.properties")
											  .getPath();
			prop.load(new FileReader(fileName));
			
		} catch (IOException e) {
			e.printStackTrace();
			throw new ProductException("ProductDAO생성자  오류 : "+e.getMessage());
		}
	}
	...
	public List<Product> selectProductList (Connection conn) throws ProductException {
		...
		try {
			...
			e.printStackTrace();//콘솔로깅용으로 남겨둠
			//사용자 정의 예외 던짐.
			throw new ProductException("selectProductList 메소드 에러! : "+e.getMessage());
	...
	public List<ProductIO> selectProductIOList (Connection conn) throws ProductException {
		...
		try {
			...
			while(rset.next()){
				...
				pio.setpDate(rset.getDate("pdate"));
			...
			e.printStackTrace();//콘솔로깅용으로 남겨둠
			//사용자 정의 예외 던짐.
			throw new ProductException("selectProductIOList 메소드 에러! : "+e.getMessage());
	...
	public Product selectOne(Connection conn, String productId) throws ProductException {
		...
		try {
			...
			e.printStackTrace();//콘솔로깅용으로 남겨둠
			//사용자 정의 예외 던짐.
			throw new ProductException("selectOne 메소드 에러! : "+e.getMessage());
	...
	public int insertProduct(Connection conn, Product p) throws ProductException {
		...
		try {
			...
			e.printStackTrace();//콘솔로깅용으로 남겨둠
			//사용자 정의 예외 던짐.
			throw new ProductException("insertProduct 메소드 에러! : "+e.getMessage());
		} finally {
			close(pstmt);
		}
	...
	public int updateProduct(Connection conn, Product p) throws ProductException {
		...
		try {
			...
			e.printStackTrace();//콘솔로깅용으로 남겨둠
			//사용자 정의 예외 던짐.
			throw new ProductException("updateProduct 메소드 에러! : "+e.getMessage());
		} finally {
			close(pstmt);
		}
	...
	public List<Product> selectByPId(Connection conn, String pId) throws ProductException {
		ArrayList<Product> list = null;
		...
		try {
			...
			e.printStackTrace();//콘솔로깅용으로 남겨둠
			//사용자 정의 예외 던짐.
			throw new ProductException("selectByPId 메소드 에러! : "+e.getMessage());
	...
	public List<ProductIO> selectProductIOByPId(Connection conn, String pid) throws ProductException {
		ArrayList<ProductIO> list = null;
		...
		String query = prop.getProperty("selectProductIOByPId");
		try {
			//1. 쿼리문을 실행할 statement객체 생성
			pstmt = conn.prepareStatement(query);
			pstmt.setString(1, pid);
