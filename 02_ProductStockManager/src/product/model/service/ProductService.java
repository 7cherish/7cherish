...
public class ProductService {
	...
	public List<Product> selectByPId(String pId) throws ProductException{
		Connection conn = getConnection();
		List<Product> list = new ProductDAO().selectByPId(conn,pId);
		close(conn);
		return list;
	}
	
	/**
	 * 상품아이디로 해당상품 입출고내역조회 메소드
	 * @return
	 * @throws ProductException
	 */
	public List<ProductIO> selectProductIOByPId(String pid) throws ProductException {
		Connection conn = getConnection();
		List<ProductIO> list = new ProductDAO().selectProductIOByPId(conn,pid);
		close(conn);
		return list;
	}
}
